...
void rotate_canvas(struct widget_rotate w) {
    static lv_color_t cbuf_tmp[w.width * w.height];
    memcpy(cbuf_tmp, cbuf, sizeof(cbuf_tmp));
    lv_img_dsc_t img;
    img.data = (void *)cbuf_tmp;
    img.header.cf = LV_IMG_CF_TRUE_COLOR;
    img.header.w = CANVAS_SIZE;
    img.header.h = LAYER_HEIGHT;

    lv_canvas_fill_bg(canvas, LVGL_BACKGROUND, LV_OPA_COVER);
    lv_canvas_transform(canvas, &img, 2700, LV_IMG_ZOOM_NONE, 0, angle, 20, 10, true);
}
